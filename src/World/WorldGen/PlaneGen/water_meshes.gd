...
const WATER = preload("res://src/World/WorldGen/PlaneGen/water_chunk.tscn")
...
func add_chunk(x: int, z: int, debug: bool = false) -> void:
	...
	add_child(chunk)
	chunks.append(chunk)
	log_debug("Added water chunk at position: %s" % chunk.global_position, "",debug)

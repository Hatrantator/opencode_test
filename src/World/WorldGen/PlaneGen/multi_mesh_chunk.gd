...
var multimesh_transforms: Array = []
...
func _ready() -> void:
	name = "%s_multi_mesh_chunk(%s,%s)" % [group_name,x, z]
	half_length = int(length / 2)
	custom_aabb = AABB(Vector3(-half_length,0, half_length),Vector3(length,64, length))
	position = Vector3(x,0,z) * length
	await add_to_global_group(group_name)
	create_multimesh()

func apply_multimesh() -> void:
	...
	for index in multimesh_transforms.size():
		multimesh.set_instance_transform(index, multimesh_transforms[index])
	multimesh.visible_instance_count = multimesh.instance_count
	log_debug("MultiMesh applied with %d instances." % multimesh.instance_count)
...
func create_multimesh() -> void:
	...
	mesh_height *= 0.5  # Center the mesh on top of the cell
	...
	for x in range(-half_length, half_length):
		for z in range(-half_length, half_length):
			var y = mesh_height  # Center the mesh on top of the cell
	...
	var total_instances = cell_pos_array.size() * instances_per_cluster
	...
	for cell_pos in cell_pos_array:
		...
		for cluster_index in range(instances_per_cluster):  # Example: 16 instances per cluster
			...
			var offset_x = randf() * 1 - (1 / 2.0)  # Random offset within the cell's x boundary
			var offset_z = randf() * 1 - (1 / 2.0)  # Random offset within the cell's z boundary
			var instance_position = Vector3(
				(x * 1) + offset_x,  # Position within the x boundary
				y,                             # Position adjusted for the mesh height
				(z * 1) + offset_z   # Position within the z boundary
			)

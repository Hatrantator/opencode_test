...
func add_chunk(x: int, z: int, debug: bool = false) -> void:
	...
	var chunk = COLLISION.instantiate()
	chunk.name = "collision_chunk(%s,%s)" % [x, z]
	# Add the chunk to the world
	add_child(chunk)
	chunk.global_position = Vector3(x, 0, z) * length
	#chunk.update_shape()
	chunks.append(chunk)
	log_debug("Added collision chunk at position: %s" % chunk.global_position, "",debug)
